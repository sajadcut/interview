...
for (const id of requiredComponents) {
  if (!componentIds.has(id)) throw new Error(`staging runtime manifest is missing required component ${id}`);
}

for (const gate of [manifest.gates?.preflight, manifest.gates?.smoke]) {
...
for (const name of requiredEnv) {
  if (!(name in env)) throw new Error(`staging environment template is missing ${name}`);
}
...
if (env.STT_PROVIDER !== "whisper-http" || env.VAD_PROVIDER !== "silero-http" || env.TTS_PROVIDER !== "local-http") {
  throw new Error("staging environment template must select real local speech providers");
}
...
console.log("✓ staging runtime manifest, fail-closed environment template, and evidence gates are consistent");
