  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <div className="mb-2 text-[10px] font-medium text-indigo-600">Senior Backend Engineer · AI sourcing agent</div>
          <h1 className="text-[26px] font-semibold tracking-tight">Sourcing</h1>
          <p className="mt-1 text-[12px] text-slate-500">
            Internal-first discovery, approved source adapters, deduplication review and explainable retrieval signals.
          </p>
        </div>
          ...
          <div className="grid gap-3 p-5 pt-4 md:grid-cols-2">
            <div className="rounded-[11px] border border-slate-100 p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {["Senior Backend Engineer", "Backend Lead", ".NET Engineer", "Platform Engineer"].map((item) => <Pill key={item} tone="blue">{item}</Pill>)}
              </div>
            </div>
            <div className="rounded-[11px] border border-slate-100 p-4">
              <div className="text-[10px] font-semibold uppercase tracking-[.12em] text-slate-400">Skills & context</div>
              <div className="mt-3 flex flex-wrap gap-2">
                {[".NET", "C#", "SQL", "Kubernetes", "production debugging", "microservices"].map((item) => <Pill key={item} tone="violet">{item}</Pill>)}
              </div>
            </div>
          </div>
          <div className="border-t border-slate-100 px-5 py-4 text-[10px] text-slate-500">
            Semantic/vector similarity is a retrieval signal only. It is never converted directly into a final hiring percentage.
          </div>
        </Panel>
          ...
          <div className="space-y-2 p-5 pt-4">
            {sources.map(([name, state, kind, note], index) => (
              <div key={name} className="flex items-start gap-3 rounded-[11px] border border-slate-100 p-3.5">
                <div className={`grid h-8 w-8 place-items-center rounded-[9px] ${index === 0 ? "bg-emerald-50 text-emerald-600" : "bg-indigo-50 text-indigo-600"}`}>
                  <Icon name={index === 0 ? "talent" : "integrations"} size={14} />
                </div>
      ...
      <Panel>
        <SectionHeader title="Discovered candidates" subtitle="Strong identity matches are deduplicated automatically; ambiguous merges require human review." action={<span className="text-[10px] font-semibold text-indigo-600">Saved view · New discoveries</span>} />
