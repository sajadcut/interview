    ...
    <main className="min-h-screen bg-slate-50 px-4 py-8 text-slate-900 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-5xl">
        <header className="mb-8 flex items-center justify-between gap-4">
          ...
          <div className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[10px] font-medium text-slate-500">Secure invitation session</div>
        </header>

        <div className="grid gap-5 lg:grid-cols-[320px_1fr]">
          <aside className="rounded-[16px] border border-slate-200 bg-white p-5 shadow-sm">
            <div className="text-[11px] font-semibold uppercase tracking-[.12em] text-slate-400">Your interview</div>
            <h1 className="mt-2 text-[22px] font-semibold tracking-tight">Senior Backend Engineer</h1>
            ...
            <div className="mt-6 space-y-1">
              {steps.map(([label, note, state], index) => (
                <div key={label} className={`flex gap-3 rounded-[11px] p-3 ${state === "current" ? "bg-indigo-50" : ""}`}>
          ...
          <section className="rounded-[16px] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            <div className="flex items-start gap-3">
              ...
              <div><div className="text-[11px] font-medium text-indigo-600">Consent & privacy</div><h2 className="mt-1 text-[24px] font-semibold tracking-tight">Before your interview starts</h2><p className="mt-2 max-w-2xl text-[12px] leading-6 text-slate-500">You should understand what is recorded, how transcript/evidence is used, what the AI interviewer can and cannot evaluate, and how to stop or ask for help.</p></div>
            </div>
            ...
            <div className="mt-6 rounded-[12px] border border-amber-100 bg-amber-50 p-4 text-[10px] leading-5 text-amber-800">This development surface demonstrates the candidate UX contract. Real invitation identity, consent persistence, device APIs and interview media are not represented as production-ready until their release gates pass.</div>

            <label className="mt-6 flex items-start gap-3 rounded-[12px] border border-slate-200 p-4">
            ...
            <div className="mt-7 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-5">
              <button className="text-[11px] font-semibold text-slate-500">Privacy details</button>
              <button className="inline-flex h-10 items-center gap-2 rounded-[10px] bg-indigo-600 px-4 text-[11px] font-semibold text-white">Continue to device check <Icon name="arrow" size={14} /></button>
            </div>
          </section>
        </div>
      </div>
