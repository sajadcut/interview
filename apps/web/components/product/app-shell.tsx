...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#f5f7fb] text-slate-900 lg:grid lg:grid-cols-[224px_minmax(0,1fr)]" dir={directionFor(locale)}>
    ...
    <div className="min-w-0">
      <header className="sticky top-0 z-30 flex h-[58px] items-center gap-3 border-b border-slate-200 bg-white/95 px-4 backdrop-blur sm:px-6"><div className="flex min-w-0 flex-1 items-center"><div className="relative hidden w-full max-w-md md:block"><Icon name="search" size={15} className="absolute start-3 top-1/2 -translate-y-1/2 text-slate-400"/><input className="h-9 w-full rounded-[9px] border border-slate-200 bg-slate-50 ps-9 pe-3 text-[11px] outline-none transition focus:border-indigo-300 focus:bg-white" placeholder="Search candidates, jobs, interviews..."/></div></div><button className="hidden h-9 items-center gap-2 rounded-[9px] border border-indigo-100 bg-indigo-50 px-3 text-[10px] font-semibold text-indigo-700 sm:inline-flex"><Icon name="sparkles" size={14}/>Ask AI</button><Link href="/app/jobs/new" className="inline-flex h-9 items-center gap-2 rounded-[9px] bg-indigo-600 px-3 text-[10px] font-semibold text-white shadow-sm"><Icon name="plus" size={14}/>Create Job</Link><button className="relative grid h-9 w-9 place-items-center rounded-[9px] border border-slate-200 bg-white text-slate-600"><Icon name="bell" size={15}/><span className="absolute end-1 top-1 h-2 w-2 rounded-full bg-rose-500 ring-2 ring-white"/></button><div className="grid h-9 w-9 place-items-center rounded-full bg-gradient-to-br from-amber-100 to-violet-200 text-[10px] font-bold">SN</div></header>
